...
class UserIos extends User {
	...
	public function initUser($param, &$pageName) {
		...
		if (isset($param['iosUid'])) {
			// UUID（iosUid）ありの場合
			if (!isset($_SESSION['iosUid'])) {
				// セッションなしの場合
				//     アプリ起動時、またはセッション切れによるリダイレクトで通る。
				// 全セッション初期化
				$_SESSION = array();
				$_SESSION['iosUid'] = $param['iosUid'];    			// UUID設定
				if (WV_DEBUG) debug($_SESSION, 'アプリ起動時の iosUid 取得成功');
			} else {
				// セッションありの場合
				// アプリを再起動したが、セッションが残留している場合、
				// ディープリンク経由で来た場合などにここを通る。
				// $_SESSION['iosUid'] = $param['iosUid'];   // iosUid が変更されることはありえない。
				if (WV_DEBUG) debug($_SESSION, 'iosUid セッション取得済み');
			}
			// セッションの有無に関係なく更新
			...
			if (!isset($_SESSION['iosUid'])) {
				...
				if (!isset($_SESSION['getUuidRedirectCnt'])) {
					...
					if (WV_DEBUG) errorLog(setLogDetail($url, $_SESSION), 'セッション切れによる iosUid 取得実行リダイレクト発生');
					header("location:{$url}");
					exit;
				} else {
					if ($_SESSION['getUuidRedirectCnt'] >= 1) {
						// 初回で取得できなければエラー
						// 2019/10/02 発生
						//     通常ありえないはずだが何故か getUuid=1&p=UpdateMap でリダイレクトして来ており、
						//     アプリ側で Page/UpdateMap にリダイレクトしようとしたため「PHPファイルが存在しません。」
						//     のエラーになりここを通った。 → 上の if (ACCESS === 'USER') により暫定対処済み。
		...
		if (!isset($_SESSION['userId'])) {
			...
			// UUIDからユーザー情報取得
			if ($dataUser = $this->getByUuid($iosUid)) {
		...
		if ($dataUser) {
			...
			if (!$dataUser['iosUid'] || $dataUser['iosUid'] !== $iosUid) {
				$paramUpdate['iosUid'] = $iosUid;
				$dataUser['iosUid'] = $iosUid;
				if (WV_DEBUG) trace("既存アカウントの iosUid 更新：{$iosUid}");
				// UsersTemp に新しい $iosUid があるはずなので、デーバイストークン取得して削除
				$usersTemp = new UsersTemp();
				$dataUserTemp = $usersTemp->getByUuid($iosUid);
				if ($dataUserTemp) {
					// アプリ側のデータ移行
					if ($dataUserTemp['deviceToken']) {
						$paramUpdate['deviceToken'] = $dataUserTemp['deviceToken'];
					}
					if ($dataUserTemp['iosInfo']) {
						$paramUpdate['iosInfo'] = $dataUserTemp['iosInfo'];
					}
			...
			if ($newSession) {
				// iosInfo 更新
				$iosInfo = json_decode($dataUser['iosInfo']);
				if ($iosInfo->osVersion !== $_SESSION['osVersion']
					|| $iosInfo->versionCode !== $_SESSION['versionCode']) {
					$paramUpdate['iosInfo'] = json_encode(
			...
			// 会員未登録の場合
			$usersTemp = new UsersTemp();
			$dataUser = $usersTemp->getByUuid($iosUid);
			if ($dataUser) {
				// UsersTemp 登録済みの場合
		...
		// バージョンチェック
		// ユーザーがアプリやOSをバージョンアップしたとして、セッションが更新されない
		// 可能性もあるので $newSession は条件にしないこと！
		// if ($newSession && !$this->checkVersion()) {
		...
		// パラメータ：showReviewAlert がある場合除外（これがないと iOS で無限ループが発生する。）
	 ...
	 * デバイストークン削除
	 *
	 * 無効なデバイストークンが検知されたら削除する。
	 *
	 */
	public function deleteDeviceToken($token) {
		// Users から検索
		$target = array();
		if ($data = $this->getByDeviceToken($token)) {
			$userDb = 'Users';
			$target['table'] = $userDb;
			...
			$userDb = 'UsersTemp';
			$usersTemp = new UsersTemp();
			$data = $usersTemp->getByDeviceToken($token);
			$target['table'] = $userDb;
			$target['userData'] = $data;
		}
		if (!$data) {
			showError($token, '削除対象の deviceToken なし');
			return;
		}
		$paramUpdate = array(
			'updTime' => array('value' => date('Y-m-d H:i:s'), 'type' => 'datetime'),
			'deviceToken' => array('value' => null, 'type' => PDO::PARAM_STR)
		);
		$paramWhere = array(
			'deviceToken' => array('value' => $token, 'type' => PDO::PARAM_STR)
		);
		if (!Db::pdoUpdate($userDb, $paramUpdate, $paramWhere)) {
			showError($target, 'エラー：デバイストークン削除失敗');
		} else {
			if (WV_DEBUG) debug($target, 'デバイストークン削除成功');
		}
	}
	 ...
	 *
	 */
	public function updateDeviceToken($uuid, $param) {
		...
		// トークンが有効かチェック
		$deviceToken = null;
		if ($param['deviceToken'] && $param['deviceToken'] !== '(null)') {
			$deviceToken = $param['deviceToken'];
		}
		if ($dataUser) {
			$userId = $dataUser['userId'];
			if ($deviceToken && $dataUser['deviceToken'] !== $deviceToken) {
				// 相違ありなら更新
				$updateParam['deviceToken'] = $deviceToken;
				$this->updateUsers($dataUser['userId'], $updateParam);
				if (WV_DEBUG) debug($dataUser['deviceToken'] . ' → ' . $deviceToken, 'Users デバイストークン更新発生');
			}
		} else {
			$usersTemp = new UsersTemp();
			...
			if ($dataUser) {
				...
				if ($deviceToken && $dataUser['deviceToken'] !== $deviceToken) {
					...
					if (WV_DEBUG) debug($dataUser['deviceToken'] . ' → ' . $deviceToken, 'UsersTemp デバイストークン更新発生');
				}
			} else {
				// UsersTemp 未登録の場合
				// デバイストークン通知の方がアプリ起動時のウェブビューロードからのアクセスより
				// 早くなった場合に発生するが、トークン通知処理にスリープを入れたので、
				// Android 1.1.1／14、iOS 1.1.1／6.1 なら発生しないはず！
				// errorLog(setLogDetail($param, $uuid), 'デバイストークン更新対象のUUID未登録検知');  // TODO：頻発するので当分停止
			}
		}
		return $userId;
	}
