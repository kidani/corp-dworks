...
class Db extends PDO {
    ...
    */
    public static function pdoUpdate($table, $paramUpdate, $paramWhere) {
        ...
        if ($result) {
        	// where 条件にマッチするものがなくても $result == ture が返るので注意。
     ...
     *
	 * 
	 * INSERT...ON DUPLICATE KEY UPDATE により 条件にあうフィールドだけ更新する。
	 * 
	 * @param    $table         ：テーブル名
	 * @param    $paramUpsert   ：データ登録・更新するカラム名と値のリスト
    ...
    */
    public static function pdoUpsert($table, $paramUpsert) {
        ...
        try {
            ...
            $PhSet = '';
            foreach($paramUpsert as $key => $value) {
                if ($key != 'insTime') {    						// 更新時は insTime を除外
    ...
    */
	 ...
	 * 
	 */
	public static function makePhInFromString($keyword, $IsString = false) {
		// 検索ワードのリスト化
		$keyword = formatSpace($keyword);		// 入力文字列の整形
		$keyList = explode(' ', $keyword);		// スペースで配列化
		return $this->makePhInFromList($keyList);;
	}
	/**
	 * 
	 * 日時条件指定のSQL作成
	 ...
	 * @param    $param			：日時条件指定するキー：searchDayFrom、searchDayTo と値からなるリスト。
	 * @param    $colTime		：日時条件指定するカラム名
	 * @return                  ：作成したフレーズ
	 * 
	 */	 
